...
function ex3Filter(products) {
  let searchProducts = [];
  for (const pro of products) {
    if (pro.price < 500 && pro.price > 100 && pro.category === "ELEC") {
      searchProducts.push(pro);
    }
  }
...
function ex4Filter(products) {
  let searchProducts2 = [];
  for (const pro of products) {
    if (pro.price < 600 && pro.price > 500 && (pro.category === "COMPUTER" || pro.category === "LAPTOP")) {
      searchProducts2.push(pro);
    }
  }
}
...
function ex9Filter(products) {
  ...
  for (const pro of products){
    let newProduct={
      ...
      "category":pro.category.substring(0,3),
      "price":pro.price + "€"
    }
    newArray.push(newProduct)
  }
...
function ex11Filter(products) {
    for(const pro of products){
      if(pro.category.toUpperCase() === "GAME" &&
...
function ex12Filter(products) {
  for(const pro of products){
    if( pro.category.toUpperCase() === "COMPUTER" &&
        pro.name.includes("presario") &&
        pro.brand.toUpperCase() === "MSI"
...
function ex13Filter(products) {
  ...
  for(const pro of products){
    if((pro.category === "COMPUTER" || pro.category === "LAPTOP") &&
      ...
      let productUpdate = (pro.price * pro.discount/100);
      pro.finalPrice = pro.price - productUpdate;
...
function ex14Filter(products) {
  let arrayProducts = [];
  for(const pro of products){
    if(!pro.brand === "HP" && pro.stock && pro.category === "COMPUTER" && !pro.onSale){
      arrayProducts.push(pro);
    }
  }
  return arrayProducts
